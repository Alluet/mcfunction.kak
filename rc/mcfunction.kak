...
provide-module mcfunction %❤
    ...
    add-highlighter shared/mcfunction/params/selkey     region '[\{\[,;][\t ]*[A-Za-z0-9_.+-]+(?:$|(?==)|[\t ]*(?=[,\]]))' '\K' regex '(?<![A-Za-z0-9_.+-])(?:(?:(true|false|[0-9]+(?:[.][0-9]*)?(?:[eE][+-]?[0-9]*)?[DdFf]?|(?:[0-9]*[.])?[0-9]+(?:[eE][+-]?[0-9]*)?[DdFfts]?|[0-9]+[BbSLl]))[\t ]*(?![\t =A-Za-z0-9_.+-])|([A-Za-z0-9_.+-]+)[\t ]*(?![\t A-Za-z0-9_.+-]))' 1:value 2:variable
    ...
    add-highlighter shared/mcfunction/params/namespaced region '(?:(?<==)[\t ]*#[a-z0-9_.-]|(?<![\t ,:\[\{\}\]"])[\t ]*#?[a-z0-9_.-]*[:/])[^,\[\{\}\]\t ]*(?:$|(?=[\t ,\[\{\}\]]))' '\K' regex '(?<![\t ,:\[\{])[\t ]*(#?(?:[a-z0-9_.-]*:)?[a-z0-9_./-]*)([^,\[\{\}\]\t a-z0-9_.-][^,\[\{\}\]\t ]*)?(?:$|(?=[\t ,\[\{\}\]]))' 1:type 2:error
